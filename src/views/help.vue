<template>
    <div class="help_page">
        ...
        <div class="search_bar">
            ...
            <!---model="searchQuery" 表示输入框中的值将会与 searchQuery 这个数据属性绑定。当用户输入内容时，searchQuery 的值会实时更新，反之亦然。  -->
            <!--@input="onSearchInput"：当用户在输入框中输入内容时，触发 onSearchInput 方法。-->
            <!--@blur="hideSearchResults"：当输入框失去焦点时，触发 hideSearchResults 方法。  -->
...
//   filteredCategories 是通过 computed 函数创建的计算属性。当 searchQuery 或 categories 发生变化时，filteredCategories 会自动重新计算
  
  const searchResults = ref(categories.value.slice());
  ...
  watch(showResults, (newShowResults) => {
      console.log('showResults changed to:', newShowResults);
  });
  </script>
